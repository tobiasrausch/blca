...
if (length(args)>1) {
 ...
 for(i in 1:nrow(seg)) {
    idx = ((x$chr==seg[i,]$chr) & (x$start >= seg[i,]$start) & (x$end <= seg[i,]$end))
    if (sum(idx)) {
      if (seg[i,]$cn < 1.5) { x[idx,]$clr = 2; }
      if (seg[i,]$cn > 2.5) { x[idx,]$clr = 3;  }
      if (seg[i,]$cn > 4)   { x[idx,]$clr = 4;  }
    }
 }
}
x$clr = factor(x$clr, levels=1:4)
print(summary(x))
...
p = p + geom_point(aes(color=clr), pch=21, fill="black", size=0.5)
p = p + xlab("Chromosome")
...
p = p + ggtitle(args[1])
p = p + scale_colour_manual(values = c("black", "blue", "red", "darkred"))
